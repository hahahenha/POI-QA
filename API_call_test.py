...
import requests
# 调用ollama接口进行中文翻译
# Call ollama for Chinese translation
...
for sample in dataset:
    ...
    # "Please predict at least {K} POIs near the vehicle's destination according to the vehicle's departure time, POIs near the starting point, and POIs along the way, in the following format: "POIs near the vehicle's destination are: \n[<name> (large category, medium category, small category),...]
    ...
    response = ""
    for txt in res_lst:
        if '车辆终点附近的POI有' in txt: # There are POIs near the vehicle destination.
            response = txt
            break
